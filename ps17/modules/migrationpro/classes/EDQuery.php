...
class EDQuery
{
    ...
    public function getCountInfo($map)
    {
        ...
        if (reset($map['entity']['orders']) == 1) {
            if (reset($map['additional']['migrate_recent_data'])) {
                $last_migrated_cust_message_id = MigrationPro::mpConfigure('latest_migrated_cust_message_id', 'get');
                $q['message_threads'] = 'SELECT count(1) as `c` FROM  `' . pSQL($this->tp) . 'customer_thread` WHERE id_customer_thread > ' . (int)$last_migrated_cust_message_id;
            } else {
    ...
    // main&general query
    $main_recent_data = 'SELECT * FROM ' . pSQL($this->tp) . 'customer_thread  WHERE id_customer_thread > ' . (int)$last_migrated_cust_message_id . ' ORDER BY id_customer_thread ASC LIMIT ' . (int)$this->row_count;
    
    $main = 'SELECT * FROM ' . pSQL($this->tp) . 'customer_thread ORDER BY id_customer_thread ASC LIMIT ' . (int)$this->offset . ',' . (int)$this->row_count;

    $q = array();
    if ($this->recent_data) {
        $q['customerThreads'] = 'SELECT * FROM ' . pSQL($this->tp) . 'customer_thread WHERE id_customer_thread > ' . (int)$last_migrated_cust_message_id . ' ORDER BY id_customer_thread ASC LIMIT ' . (int)$this->row_count;

        $q['customerMessages'] = 'SELECT DISTINCT  * FROM ' . pSQL($this->tp) . 'customer_message m
                                    INNER JOIN ( ' . $main_recent_data . ') AS t ON m.id_customer_thread = t.id_customer_thread ORDER BY m.id_customer_thread ASC';
    }else{
